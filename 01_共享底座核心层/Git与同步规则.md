...
- 新增的 `07_Codex生成区` 也属于文档库正常目录
- 你后续让我生成的文件，只要放在这个目录下且没有命中 `.gitignore`，也会同步
- 这个目录更适合放过程产物、截图整理结果、临时稿、导出稿，而不是长期稳定的模块主文档
...
- 社区插件安装包本体：不会同步
- 插件配置文件：大部分会同步
- `07_Codex生成区` 下的新建文件和分类目录：默认会同步
- `07_Codex生成区` 下的截图、图片整理结果：默认会同步
