...
当前这套工作区后续默认遵守“强归位”原则。

也就是说：

- 不允许把新文件长期散落在 `D:\AI_workspace\codex` 根目录
- 新内容必须优先判断属于哪一个功能区
- 如果只是临时生成但仍有保存价值，也应进入对应的分类目录，而不是先乱放

当前推荐归位规则是：

- 正式共享规则、结构说明：放 `01_共享底座核心层`
- 模块入口文档：放 `02_模块文档`
- 规则共建文档：放 `03_规则文档`
- 需求共建文档：放 `04_需求文档`
- 前置分析：放 `05_多智能体前置分析`
- 会话续接与归档：放 `06_会话摘要与归档`
- Codex 生成文件、截图整理结果、临时产物：放 `07_Codex生成区`
- 工具脚本、依赖、历史排查脚本：放 `D:\AI_workspace\codex\tools`
- 原始资料来源：放 `D:\AI_workspace\codex\10_知识库导入`

后续如果我再为你生成文件，我会优先按这套规则归位，不再把文件随手放在根目录。
